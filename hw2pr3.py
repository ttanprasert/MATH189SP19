...
def linreg(X, y, reg=0.0):
	...
	I = np.eye(X.shape[1])
	I[0][0] = 0
	W_opt = np.linalg.solve(X.T @ X + reg * I, X.T @ y) # Formula from part (b)
	"*** END YOUR CODE HERE ***"
	return W_opt
...
def find_RMSE(W, X, y):
	...
	"*** YOUR CODE HERE ***"
	dy = y - predict(W, X)
	RMSE = np.sqrt(np.linalg.norm(dy, 2)**2 / X.shape[0])
	"*** END YOUR CODE HERE ***"
	return RMSE
...
def RMSE_vs_lambda(X_train, y_train, X_val, y_val):
	...
	W_list = [linreg(X_train, y_train, reg = l) for l in reg_list]
...
def linreg_no_bias(X, y, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	m = X.shape[0]
	temp = X.T @ (np.eye(m) - (np.ones(m)/m))
	W_opt = np.linalg.solve(temp @ X + reg * np.eye(X.shape[1]), temp @ y)
...
def grad_descent(X_train, y_train, X_val, y_val, reg=0.0, lr_W=2.5e-12, \
	...
	while np.linalg.norm(W_grad) > eps and np.linalg.norm(b_grad) > eps \
		...
		"*** YOUR CODE HERE ***"
		train_rmse = np.sqrt(np.linalg.norm((X_train @ W).reshape((-1, 1))-y_train+b)**2 / m_train)
		obj_train.append(train_rmse)
		obj_val.append(np.sqrt(np.linalg.norm((X_val @ W).reshape((-1, 1))-y_val+b)**2 / m_val))
		W_grad = ( (X_train.T @ X_train + (reg*np.eye(n)) ) @ W + X_train.T @ (-y_train+b)) / m_train
