...
def NLL(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	pred_y = sigmoid(X @ W)
	one_nll1 = np.multiply(y, np.log(pred_y))
	one_nll2 = np.multiply((1.-y), np.log(1.-pred_y))
	nll_temp = -sum(one_nll1+one_nll2)
	nll = nll_temp + (reg/2) * (np.linalg.norm(W)**2)
	nll = nll[0]
	"*** END YOUR CODE HERE ***"
	return nll
...
def newton_step(X, y, W, reg=0.0):
	...
	grad = grad_logreg(X, y, W, reg)
	mu = sigmoid(X @ W)
	diag = np.diag(np.squeeze(np.multiply(mu, 1.-mu)))
	temp = reg*np.eye(X.shape[1])
	H = X.T @ diag @ X + temp
	...
	for i in range(X.shape[0]):
		if y[i] == pred_y[i]:
			...
			if y[i] == 1 and pred_y[i] == 1:
				ones = ones+1
	accuracy = 1.*accuracy/X.shape[0]
	precision = 1.*	ones / sum([x for x in pred_y if x == 1])
	recall = 1.* ones / sum([x for x in y if x == 1])
...
def plot_description(X_train, y_train, X_test, y_test):
	...
	for r in reg_list:
		W_opt, obj = grad_descent(X_train, y_train, reg=r, lr = 2e-4)
